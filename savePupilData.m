...
%   by: zvi roth
%   date: 9/3/2020
%   purpose: save pupillometry data
%

function[] = savePupilData()

saveFolder = '/Volumes/TaskDrive/taskRelatedBehavioral/';%change this to the folder containing all subjects' eye-tracking data
...
for iSub = 1:length(subFolders)
    ...
    for iRun=1:length(stimfiles)
        stimfile=stimfiles(iRun).name;%get stimfile filename
        s{iRun}=load(stimfile);%load stimfile
