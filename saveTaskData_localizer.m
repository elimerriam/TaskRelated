% saveTaskData_localizer
% 
% associated with the following publication: Roth ZN, Ryoo M, and Merriam EP (2020). 
% Task-related activity in human visual cortex. 
%
% creates data file used by: permutationTest.m
%
%   usage: saveTaskData_localizer()
%   by: zvi roth
%   date: 9/6/2020
%   purpose: save fMRI data, excluding voxels that respond to stimulus
%   localizer
%

function[] = saveTaskData_localizer()

dataFolder = '/Volumes/TaskDrive/min/';%folder containing all fMRI data
saveFolder = '~/min/test/';%data will be saved here
removeLocalizer = 1;%0=don't remove voxels identified by the localizer. 1 = remove localizer voxels.
...
if removeLocalizer~=0%localizer is used only for EVC
...
for iSub = 1:numSubs
    ...
    cd(subFolders{iSub});
    v=newView;%create new mrLoadRet view
    ...
    for iScan = 1:2%nScans%2 concatenations, 1 for each reward type
        ...
        for iRoi = 1:length(roiNames)
            ...
            locPh{iSub,iRoi}=[];
            locAmp{iSub,iRoi}=[];
            origVoxNum(iSub,iRoi) = size(roiTC{iSub,iRoi,rwd}.tSeries,1);
            if removeLocalizer~=0 && iSub~=4 %subject 4 has no localizer scan
                %get localizer data
                avgGroupName = 'Averages';
                locScan = 3;
                matchScan = 1;
                locCorrData = loadROIcoranalMatching(v, roiNames{iRoi}, locScan, avgGroupName, matchScan, concatGroupNum);
                locCo{iSub,iRoi} = locCorrData{1}.co;
                locPh{iSub,iRoi} = locCorrData{1}.ph;
                locAmp{iSub,iRoi} = locCorrData{1}.amp;                
                if removeLocalizer==1
                    %keep only voxels that don't respond to localizer above
                    %threshold
                    roiTC{iSub,iRoi,rwd}.tSeries = roiTC{iSub,iRoi,rwd}.tSeries(locCo{iSub,iRoi}<locThresh,:);
                elseif removeLocalizer==-1
                    %keep only voxels that  respond to localizer above threshold
                    roiTC{iSub,iRoi,rwd}.tSeries = roiTC{iSub,iRoi,rwd}.tSeries(locCo{iSub,iRoi}>locThresh,:);
                end
            end
            finalVoxNum(iSub,iRoi) = size(roiTC{iSub,iRoi,rwd}.tSeries,1);
            locPercent(iSub,iRoi) = 100*finalVoxNum(iSub,iRoi)./origVoxNum(iSub,iRoi);
            if toZscore
    ...
    for iRoi = 1:length(roiNames)
        ...
        if removeLocalizer~=0 && iSub~=4 %subject 4 has no localizer scan
            if removeLocalizer==1
                eccen{iSub,iRoi} = eccen{iSub,iRoi}(locCo{iSub,iRoi}<locThresh);
                ang{iSub,iRoi} = ang{iSub,iRoi}(locCo{iSub,iRoi}<locThresh);
                areas{iSub,iRoi} = areas{iSub,iRoi}(locCo{iSub,iRoi}<locThresh);
            elseif removeLocalizer==-1
                eccen{iSub,iRoi} = eccen{iSub,iRoi}(locCo{iSub,iRoi}>locThresh);
                ang{iSub,iRoi} = ang{iSub,iRoi}(locCo{iSub,iRoi}>locThresh);
                areas{iSub,iRoi} = areas{iSub,iRoi}(locCo{iSub,iRoi}>locThresh);
...
%%
save([saveFolder 'rwdTC_concat' onlyCorrectString zScoreString ConcatProjStr noLocStr '.mat'], 'concatInfo',  'subResponse', 'roiMeanTseries', ...
    'roiTC', 'allTrials', ...
    ...
    'subMeanRunTC','subStdRunTC','subStd','subRoiRuns','runRwd',...
